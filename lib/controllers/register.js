...
const outgoingHDPublicKey = new bitcore.HDPublicKey(chains.outgoingPublicKey(), chains.network())
/**
...
const register = async (hash) => {
  ...
  if (docAddress) {
    ...
    //const paymentAddress = new bitcore.Address(outgoingHDPublicKey.publicKey, chains.network());
    const paymentAddress = incomingHDPrivateKey.privateKey.toAddress()
