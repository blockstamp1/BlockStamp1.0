...
const incomingHDPrivateKey = new bitcore.HDPrivateKey(chains.incomingPrivateKey(), chains.network())
const outgoingHDPublicKey  = bitcore.HDPublicKey(incomingHDPrivateKey)
...
const register = async (hash) => {
  ...
  if (docAddress) {
    ...
    console.log("RPATH:", randomPath)
    console.log("HDPRV:", incomingHDPrivateKey)
    console.log("HDPUB:", outgoingHDPublicKey);
		const paymentAddress = new bitcore.Address(hdPublicKey.publicKey)
    console.log("Address:", paymentAddress.toString());
