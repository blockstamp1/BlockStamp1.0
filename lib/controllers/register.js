...
const register = async (hash) => {
  ...
  if (docAddress) {
    ...
    // var hdpath_hard = "m/44'/0'/0'";
    //const paymentAddress = incomingHDPrivateKey.derive(randomPath).privateKey.toAddress();
    // var derivedKey = incomingHDPrivateKey.derive(hdpath_hard)
    // var hdpub = derivedKey.hdPublicKey;
		const paymentAddress = new bitcore.Address(chains.outgoingPublicKey(), chains.network());
