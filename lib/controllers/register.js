...
let incomingHDPrivateKey = new bitcore.HDPrivateKey(chains.incomingPrivateKey())
...
const register = async (hash) => {
  ...
  if (docAddress) {
    ...
    let paymentAddress = ""
    try{
    //  paymentAddress = incomingHDPrivateKey.toString()
    //  console.log("PAY:", paymentAddress) 
     //paymentAddress = incomingHDPrivateKey.derive(randomPath).privateKey.toAddress()
     //paymentAddress = incomingHDPrivateKey.derive(randomPath).privateKey.toAddress()
     paymentAddress = incomingHDPrivateKey.derive(randomPath).toString()
    } catch(err){
      console.log("Error1:", err)
      console.log("Error:", err.message, ":", err.toString())
    }
    try{
      paymentAddress = incomingHDPrivateKey.derive(randomPath).privateKey.toString();
    } catch(err){
      console.log("Error1:", err)
